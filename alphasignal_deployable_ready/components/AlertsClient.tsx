...
export function AlertsClient() {
  ...
  function addAlert(event: React.FormEvent) {
    ...
    setItems(updated); alertsStore.set(updated); setTriggerPrice(0);
  }

  function removeAlert(id: string) {
    const updated = items.filter((item) => item.id !== id);
    setItems(updated); alertsStore.set(updated);
  }
  ...
  return (
    <div className="grid-2">
      <div className="card">
        ...
        <p>In this deployable version, alerts are stored locally in the browser. Swap to database + email delivery when you wire in Supabase and cron jobs.</p>
      ...
      <div className="card">
        ...
        {items.length === 0 ? (
          <div className="empty">No alerts yet. Add one threshold and convert noise into a decision trigger.</div>
        ) : (
